...
function LargeButton({
  ...
  const isSplit = btn.variant === "split-large" && !!onSplit;
  const autoW = btn.customWidth == null;
  const h = btn.customHeight ?? LARGE_HEIGHT;
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex items-center justify-center gap-0.5 px-0.5 w-full", showIcon ? "mt-1" : "flex-1")}>
        ...
        {ready && isSplit && (
          <ChevronDown
            className="h-2.5 w-2.5 shrink-0 text-inventor-text-muted"
            role="button"
...
function SmallButton({
  ...
  const isSplit = btn.variant === "split-small" && !!onSplit;
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={btn.label.replace(/\n/g, " ")}
      style={autoW ? { height: h } : { width: btn.customWidthSmall, height: h }}
      className={cn(
    ...
    >
      ...
      {ready && isSplit && (
        <ChevronDown
          className="h-2.5 w-2.5 text-inventor-text-muted shrink-0"
          role="button"
          aria-label="More tools"
          onClick={(e) => {
            e.stopPropagation();
...
function Group({
  ...
  const secondary = group.priority === "secondary";

  const body = (
    <div
      className={cn(
        "flex items-stretch gap-0.5 pt-1.5 flex-1",
        secondary ? "px-2 opacity-95" : "px-2.5",
      )}
    >
      {group.columns.map((col, i) => (
        <Fragment key={i}>
          <Column
            ...
            ready={ready}
            onClick={onClick}
            onSplit={hasDropdown ? () => setOpen(true) : undefined}
          />
  ...
  const labelCls = cn(
    "text-center text-[11px] tracking-wide mt-1 pt-0.5 pb-1 px-2",
    secondary ? "text-inventor-text-muted/80" : "text-inventor-text-muted",
  );
  ...
  if (!hasDropdown) {
    return (
      <div className="flex flex-col justify-between">
        {body}
        <div className={labelCls}>{group.name}</div>
      </div>
    );
  }
    ...
    <div className="flex flex-col justify-between">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          align="start"
          sideOffset={0}
          className="w-auto min-w-[var(--radix-popover-trigger-width)] p-1 rounded-none border border-inventor-ribbon-border bg-inventor-ribbon"
        >
...
export function Ribbon({
  ...
  // Scroll affordance — fade the right edge while content is hidden.
  const scrollRef = useRef<HTMLDivElement | null>(null);
  const [moreRight, setMoreRight] = useState(false);
  const measure = useCallback(() => {
    const el = scrollRef.current;
    if (!el) return;
    setMoreRight(el.scrollWidth - el.clientWidth - el.scrollLeft > 2);
  }, []);
  useEffect(() => {
    measure();
    const el = scrollRef.current;
    ...
    <div className="border-b border-inventor-ribbon-border bg-inventor-ribbon select-none">
      ...
      <div className="relative">
        <div
          ref={scrollRef}
          onScroll={measure}
          className="flex items-stretch overflow-x-auto min-h-[92px] w-full"
        >
          {currentTab?.groups.map((group, gi) => (
            <Fragment key={group.id}>
              ...
              {gi < currentTab.groups.length - 1 && (
                <div className="w-px shrink-0 bg-inventor-ribbon-border my-1" />
              )}
            </Fragment>
          ))}
        ...
        {moreRight && (
          <div className="pointer-events-none absolute inset-y-0 right-0 w-10 bg-gradient-to-l from-inventor-ribbon to-transparent" />
        )}
      </div>
    </div>
  );
